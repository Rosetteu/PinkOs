...
_G.graphic = {
    ...
    drawString = function(String, x, y, textColor, backgroundColor)
        ...
        if x == enum.UiPosition.center then
            x = math.floor((graphic.screenSize[1] - string.len(String)) / 2) + 1
        end
        if not (x > 0 and x <= graphic.screenSize[1]) then
            log.add(enum.logType.error, "Param #2 x isn't between 0 and " .. graphic.screenSize[1], fileName, false)
            return false, "Param #2 x isn't between 0 and " .. graphic.screenSize[1]
        end
        ...
        if y == enum.UiPosition.center then
            y = math.floor(graphic.screenSize[2] / 2 + 1)
        end
        if not (y > 0 and y <= graphic.screenSize[2]) then
            log.add(enum.logType.error, "Param #3 y isn't between 0 and " .. graphic.screenSize[2], fileName, false)
            return false, "Param #3 y isn't between 0 and " .. graphic.screenSize[2]
        end
    ...
    createButton = function(Function, id, String, x, y, textColor, backgroundColor, oneUse)
        ...
        local success, err = graphic.drawString(String, x, y, textColor, backgroundColor)
        if not success then
    ...
    deleteButton = function(id)
        log.add(enum.logType.debug, "function deleteButton called with arguments {id=\"" .. id .. "\"}")
    ...
    drawSquare = function(x, y, width, height, color)
        ...
        if x == enum.UiPosition.center then
            x = math.floor(graphic.screenSize[1] / 2 - width / 2)
        end
        if not (x > 0 and x <= graphic.screenSize[1]) then
            log.add(enum.logType.error, "Param #2 x isn't between 0 and " .. graphic.screenSize[1], fileName, false)
            return false, "Param #2 x isn't between 0 and " .. graphic.screenSize[1]
        ...
        if y == enum.UiPosition.center then
            y = math.floor(graphic.screenSize[2] / 2 - height / 2) + 1
        end
        if not (y > 0 and y <= graphic.screenSize[2]) then
            log.add(enum.logType.error, "Param #2 y isn't between 0 and " .. graphic.screenSize[2], fileName, false)
            return false, "Param #2 y isn't between 0 and " .. graphic.screenSize[2]
        end
        ...
        if not (width > 0 and tonumber(width) + tonumber(x) <= graphic.screenSize[1]) then
        ...
        if not (height > 0 and tonumber(height) + tonumber(y) <= graphic.screenSize[2]) then
        ...
        paintutils.drawFilledBox(tonumber(x), tonumber(y), tonumber(x) + tonumber(width) - 1,
            tonumber(y) + tonumber(height) - 1, color)
...
graphic.deletteButton = graphic.deleteButton
_G.grafic = graphic
